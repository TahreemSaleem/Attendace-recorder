...
width: 100%;
	...
	<?php
		...
		$query = "SELECT * FROM attendance WHERE classid = 'CS'" ;
		...
		if($_SERVER['REQUEST_METHOD'] === 'POST') 
		{
		    ...
		    {
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('CS', '$g', 1,CURDATE())";
				$result = mysqli_query($conn,$sql);
		    }
		    else{
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('CS', '$g', 0,CURDATE())";
				$result = mysqli_query($conn,$sql);
			}
		     if (isset($_POST['check3']))
		    {
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('CS', '$h', 1,CURDATE())";
				$result = mysqli_query($conn,$sql);
		    		
		    }
		    else{
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('CS', '$h', 0,CURDATE())";
				$result = mysqli_query($conn,$sql);
			}
		     if (isset($_POST['check4']))
		    {
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('CS', '$i', 1,CURDATE())";
				$result = mysqli_query($conn,$sql);
		    		
		    }
		    else{
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('CS', '$i', 0,CURDATE())";
				$result = mysqli_query($conn,$sql);
			}
			$conn->close();
