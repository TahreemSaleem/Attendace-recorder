...
width: 100%;
	...
	<?php
		...
		$query = "SELECT * FROM attendance WHERE classid = 'SE'" ;
		...
		if($_SERVER['REQUEST_METHOD'] === 'POST') 
		{
		    ...
		    {
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('SE', '$g', 1,CURDATE())";
				$result = mysqli_query($conn,$sql);
		    }
		    else{
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('SE', '$g', 0,CURDATE())";
				$result = mysqli_query($conn,$sql);
			}
		     if (isset($_POST['check3']))
		    {
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('SE', '$h', 1,CURDATE())";
				$result = mysqli_query($conn,$sql);
		    		
		    }
		    else{
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('SE', '$h', 0,CURDATE())";
				$result = mysqli_query($conn,$sql);
			}
		     if (isset($_POST['check4']))
		    {
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('SE', '$i', 1,CURDATE())";
				$result = mysqli_query($conn,$sql);
		    		
		    }
		    else{
		    	$sql = "INSERT INTO attendance (classid, studentid, isPresent,date_taken)
				VALUES ('SE', '$i', 0,CURDATE())";
				$result = mysqli_query($conn,$sql);
			}
			$conn->close();
